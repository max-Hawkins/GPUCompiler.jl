...
NVVM_REFLECT_FUNCTION = "__nvvm_reflect"
function nvvm_reflect!(fun::LLVM.Function)
    ...
    for use in uses(reflect_function)
        ...
        reflect_val = if reflect_arg == "__CUDA_FTZ"
            # XXX: put the FTZ property in the compiler job?
            #      and/or first set the "nvvm-reflect-ftz" module flag like Clang does?
            ConstantInt(reflect_typ, Base.JLOptions().fast_math)
        elseif reflect_arg == "__CUDA_ARCH"
            ConstantInt(reflect_typ, job.target.cap.major*100 + job.target.cap.minor*10)
        else
            error("Unknown __nvvm_reflect argument: $reflect_arg")
        end
